...
{/* 文章维护说明：记录 UWP 多线程问题及解决方式，示例代码以 C#/XAML 为主。 */}
...
async void CalAns()
{
    ...
    // 这个示例只解决“跨线程更新 UI”的问题，不解决“耗时计算阻塞 UI”的问题。
    int ans = 0;
    for (var i = 0; i < Int32.MaxValue; i++)
    {
        ans++;
...
async void CalAnsAsync()
{
    ...
    // 如果调用时不在 UI 同步上下文中，await 后续是否回到 UI 线程取决于当前 SynchronizationContext。
