...
{/* 文章维护说明：命令速查类内容，代码块注释应说明命令用途和风险。 */}
...
# 注意：这种写法无法安全处理带空格/换行的文件名，正式脚本建议使用 find -print0/read -d。
...
# Remove-Item *.dll 会删除当前目录全部 DLL，执行前应确认工作目录正确。
Remove-Item *.dll
...
# 这里的地址需要替换成自己的代理服务；空用户名密码写法并不适用于所有代理。
...
# convert 的通配符输出在不同 ImageMagick 版本中行为可能不同，大量文件建议用 mogrify 或循环处理。
...
# -c copy 要求输入编码能被目标容器支持，否则需要显式指定编码器重新编码。
