...
{/* 文章维护说明：UWP/MVVM Light 入门笔记，代码块保留原始 C#/XAML 示例。 */}
...
```csharp
// 通知UI层：名为"Name"的属性已经发生变化，需要刷新绑定了该属性的控件
// Lambda 写法比字符串 "Name" 更安全，重命名属性时更容易被工具发现。
...
```csharp
// 设置IoC容器的提供者为SimpleIoc，这是整个依赖注入体系的入口
ServiceLocator.SetLocatorProvider(() => SimpleIoc.Default);
// 将AppViewModel类型注册到IoC容器中，后续可通过GetInstance获取其实例
// ViewModelLocator 通常会在 XAML 中暴露属性，让页面通过 Binding 获取对应 ViewModel。
...
```xml
<CheckBox>
    <!-- 借助Interaction库，将控件的事件转换为Command绑定，实现MVVM模式下的解耦 -->
    <i:Interaction.Triggers>
        <!-- 监听Checked事件，触发时执行绑定到CheckedCommand的RelayCommand -->
        <i:EventTrigger EventName="Checked">
            <!-- Command 指向 ViewModel 中的 CheckedCommand，避免在 code-behind 中写事件处理。 -->
            <command:EventToCommand Command="{Binding CheckedCommand}"></command:EventToCommand>
        </i:EventTrigger>
        <!-- 监听Unchecked事件，触发时执行绑定到UnCheckedCommand的RelayCommand -->
...
```csharp
// 注册一个消息接收者：当收到SendMessageToken标记的string类型消息时，执行Lambda更新Msg
Messenger.Default.Register<string>(this, MessageToken.SendMessageToken, (msg) =>
        {
            // msg 是发送方传来的消息内容；这里直接写入可绑定属性。
            Msg = msg;
        });
// token 大概只是个确定特定接受方的字符串
// 发送一条string类型的消息，所有注册了对应Token的接收者都会收到此消息
// Token 可以把同一消息类型拆成多个频道，避免不同模块误收消息。
...
```csharp
// 在App启动时初始化DispatcherHelper，使其关联到UI线程的Dispatcher
DispatcherHelper.Initialize();

// 在非UI线程中调用此方法，将操作调度到UI线程执行，避免跨线程访问UI控件时的异常
DispatcherHelper.CheckBeginInvokeOnUI(() =>
                  {
                      // 集合绑定到 UI 时，修改集合也属于 UI 相关操作，需要切回 UI 线程。
