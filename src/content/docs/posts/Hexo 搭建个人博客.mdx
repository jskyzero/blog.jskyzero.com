...
date: 2018-11-11
category: "软件工具"
image: "/images/posts/new_year.jpg"
tags: ["Tutorial", "Hexo"]
---
{/* 文章维护说明：历史 Hexo 建站教程，命令示例保留用于迁移参考。 */}
...
```shell
# -g 表示全局安装 hexo 命令；如果不想污染全局环境，也可以使用 npx hexo。
...
# 在指定目录生成 Hexo 项目骨架。
hexo init <floder>
# 进入项目目录；这里的 floder 是原文拼写，实际命令中替换成你的目录名即可。
cd <floder>
...
```shell
# 启动本地开发服务器，默认监听 http://localhost:4000。
hexo server
```
...
# 将其挪到 themes 目录下
# 注意保留主题目录名，后续 _config.yml 中的 theme 要与目录名一致。
