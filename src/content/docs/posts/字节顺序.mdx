...
date: 2018-11-11
category: "计算机科学"
tags: ["Computer Science", "Endianness"]
---

{/* 文章维护说明：端序概念笔记，示例代码和输出块应突出字节排列差异。 */}
...
```c
#include <stdio.c> // 引入 printf() 函数，用于格式化输出；实际代码应为 <stdio.h>。

int main() {
    // main函数：程序的入口点，本程序的目的是检测当前机器的字节序
    
    // 假定 int 为 4 byte；不同平台上 int 宽度可能不同，严谨代码可用 sizeof(int) 判断。
    ...
    for (int i = 0; i < 4; i++) {
        // %p 打印指针地址；这里用 %c 只适合可见字符，观察十六进制时更推荐 %02x。
        printf("%p:%c\n", b, *b);
        // 指针向前移动1个字节，指向下一个字节
        b++;
    }
