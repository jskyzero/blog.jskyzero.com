...
date: 2018-11-11
category: "计算机科学"
tags: ["Computer Science", "Encode"]
---

{/* 文章维护说明：字符编码知识笔记，示例表格和代码块用于解释编码差异。 */}
...
#define UCS_SIZE 2  // UCS-2 每个码元固定 2 字节。
#define UTF_SIZE 3  // 本示例只处理 BMP 字符，因此 UTF-8 最多 3 字节。

void UCS_2_to_UTF_8(FILE * fp) {
    FILE * out = fopen("UTF-8", "w"); // 输出目标文件；实际处理二进制建议使用 "wb"。

    char UCS[UCS_SIZE]; // 暂存一个 UCS-2 码元的两个字节。
    char UTF[UTF_SIZE]; // 暂存转换后的 UTF-8 字节序列。
    size_t UTF_length;  // 当前 Unicode 码点编码为 UTF-8 后占用的字节数。
    unsigned long Unicode; // 当前 UCS-2 码元还原出的 Unicode 码点。

    // check the file is Bid Endian or Little Endian
    // Big Endian: FE FF；Little Endian: FF FE；true means Big Endian.
    // 注意：连续 fgetc 会消费 BOM 的两个字节，后续读取正文从第三字节开始。
    ...
    for (size_t i = 0; i < UCS_SIZE && !feof(fp); i++) {
        UCS[i] = fgetc(fp); // 每次读取 UCS-2 的一个字节。
    }
    ...
    // write UTF-8 to file
    if(endian) {
        // UTF-8 本身按字节序列定义，不需要随平台端序翻转。
        for (size_t i = 0; i < UTF_length; i++) {
            fputc((unsigned char)UTF[i], out);
        }
    } else {
        // 这里保留原示例逻辑；严谨实现中 UTF-8 输出仍应按 UTF[0..UTF_length) 顺序写出。
...
#define UCS_SIZE 2  // 输出 UCS-2：每个字符两个字节。
#define UTF_SIZE 3  // 示例只处理 1-3 字节 UTF-8 序列。

void UTF_8_to_UCS2(FILE * fp ) {
   
    FILE * out = fopen("UCS-2", "w"); // 输出目标文件；二进制数据建议使用 "wb"。
    
    char UCS[UCS_SIZE] = { 0xFE, 0xFF }; // 初始化为大端 BOM。
    char UTF[UTF_SIZE]; // 暂存从输入读取的 UTF-8 字节。
    size_t UTF_length;  // 当前 UTF-8 字符长度。
    unsigned long Unicode; // 解码后的 Unicode 码点。
    int break_flag = 0; 

    // out the file  Bid Endian or Little Endian
    // Bid Endian : FE FF Little Endian FF FE,  this is  Bid Endian
    // 先写 BOM，明确输出文件采用 UCS-2 Big Endian。
    ...
    if  (0 == ((unsigned char)UTF[0] >> 7)) {
        ...
        // 110 开头表示两字节 UTF-8，需要再读一个续字节。
        UTF_length = 2;
        UTF[1] = fgetc(fp);
        // 去掉 UTF-8 头部标记位，只保留有效 payload 位再拼接。
        ...
        // 1110 开头表示三字节 UTF-8，对 BMP 内大部分中文字符足够。
        UTF_length = 3;
        UTF[1] = fgetc(fp);
        UTF[2] = fgetc(fp);
    ...
    // 输出 Big Endian UCS-2：高 8 位先写，低 8 位后写。
